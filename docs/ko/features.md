...
`"BASE/QUOTE"`는 **QUOTE로 표시한 BASE의 가격**으로 읽습니다 — 즉 "사려는 자산"을 "지불하려는 자산"으로 나눈 값입니다. 따라서 `"BTC/CRO"`는 "BTC 1개가 몇 CRO인가"를 뜻하며, 이는 CRO를 BTC로 바꾸기 전에 **낮을수록 좋은** 바로 그 숫자입니다.

### 방향은 항상 하나로

거래소는 시장의 어느 쪽을 상장할지 마음대로 정합니다. crypto.com에서 CRO는 `CRO_BTC`의 **기준 통화**이지만 `ETH_CRO`에서는 **표시 통화**입니다 — 그대로 읽으면 두 페어는 **정반대** 방향을 가리킵니다. CRO를 BTC로 바꾸려면 `CRO_BTC`가 높아야 하고, CRO를 ETH로 바꾸려면 `ETH_CRO`가 낮아야 하는 식입니다. 한 브리핑에 두 가지 극성이 섞이면 오독하기 쉽고, 페어를 추가할수록 더 나빠집니다.

Richfolio는 모든 페어를 "지불하는 통화로 표시한, 사려는 자산"으로 정규화하므로 **낮음 = 저렴함 = 교환하기 좋은 시점**이 항상 성립합니다. 거래소가 어느 쪽을 상장했는지는 거래소 자체의 종목 메타데이터로 판별하며, 필요하면 시계열을 역수로 변환합니다. 페어 추가는 설정만 바꾸면 됩니다 — `"SOL/CRO"`, `"BTC/USDT"`, `"ETH/BTC"` 모두 그대로 동작합니다.
...
| 52주 범위 | 365개의 일봉에서 산출 (암호화폐는 달력일 기준 매일 거래되므로 약 252일이 아님) |
| P/E, 펀더멘털, 배당, 실적, 애널리스트 목표가 | 코인 페어에는 **애초에 존재하지 않습니다** — AI에도 명시하여 가치 등급을 지어내지 않습니다 |
| 배분 목표 / 갭 | 없음 — `watching` 목록과 동일하게 관찰 전용 |
| `suggestedBuyValue` | 항상 0 — 현금 지출이 아니라 교환이기 때문입니다 |
| 공개 소셜 게시 | 소셜 게시를 켜 두었더라도 절대 게시하지 않습니다 |

P/E가 존재할 수 없으므로 교차 페어에서 쓸 수 있는 가격 수준 신호는 셋이 아니라 **둘**뿐입니다: 52주 위치 < 30%, 그리고 200일 이동평균선 아래. P/E가 없다는 것은 **탈락 사유가 아니라고** 프롬프트에서 AI에 명시합니다.

### 신호 읽는 법

교차 페어 추천은 **교환** 신호이므로 액션의 의미가 조금 다릅니다.
...
양쪽 모두 변동성이 크기 때문에, 유리한 페어 가격은 기준 통화의 하락 **또는** 표시 통화의 상승 어느 쪽에서도 생길 수 있습니다. 데이터가 뒷받침되면 AI가 어느 쪽인지 밝히도록 요구합니다.

### 실행 주기

교차 페어는 일일 브리핑의 워치 리스트에 표시되며, 동시에 3시간마다(하루 8회) 자체 스케줄로도 실행됩니다 — 암호화폐는 24시간 내내 거래되는 반면 주식 장중 점검은 대부분 미국 시장이 닫혀 있을 때 실행되므로 높은 주기가 의미를 갖습니다.

다만 일봉은 하루에 한 번만 마감되므로, 3시간 간격의 두 실행에서 **기술적 지표는 완전히 동일합니다**. 가격 변동 없는 액션 전환은 신호가 아니라 점수 노이즈이며, `cryptoAlerts.minPriceMovePctToAlert`(기본 1.0%)가 이를 억제합니다.

스키마는 [설정 → 암호화폐 교차 페어](configuration#암호화폐-교차-페어)를 참고하세요.
