...
// e.g. "G 83 · C 80 · M 83". Returns null when the rec isn't multi-AI (a
// single-provider rec has nothing to break down) so callers can render
// nothing rather than a degenerate one-entry line.
export function formatCompactScores(rec: Pick<AIBuyRecommendation, "providers">): string | null {
  if (!isMultiAI(rec)) return null;
  return rec.providers!.map((p) => `${p.providerShortLabel} ${p.confidence}`).join(" · ");
}
