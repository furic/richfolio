// ── Pure technical-indicator layer ──────────────────────────────────
// Every function here is pure: plain arrays in, numbers out, no I/O and no
// config. That matters for two reasons.
//
// 1. Testability. `fetchTechnicals.ts` imports `config.js`, which reads
//    config.json at module load and throws when it is absent — and CI runs
//    without one. Anything reachable only through `fetchTechnicals.ts` is
//    therefore untestable, which is why these indicators had no coverage at all
//    before they lived here. Same split as allocation.ts/analyze.ts and
//    socialContent.ts/social.ts.
// 2. Source independence. The indicators are defined on a close/OHLCV series,
//    not on Yahoo's response shape. Feeding them crypto.com candles (or any
//    other feed) needs no new code — see `src/fetchCrypto.ts`.
//
// Callers are responsible for handing over candles that are already in a single
// unit: `fetchTechnicals` applies its FX rate before calling in, crypto pairs
// arrive natively denominated. There is deliberately no `rate` parameter here.

import { resolveTrendPrice } from "./util.js";

// Neutral OHLCV candle. Nullable fields mirror Yahoo's chart rows; every
// consumer below filters or guards them.
...
export function computeOBVTrend(
...
// ── Compute the full indicator set from a candle series ─────────────
// `candles` must be ascending chronological and already in a single unit.
// `spotPrice` is an optional fresher price used only for trend position.
// Returns null (with a warning) when there is not enough history.
export function computeTechnicals(
  ...
  if (!candles || candles.length < 50) {
    console.warn(
      `  ⚠ ${ticker}: insufficient chart data (${candles?.length ?? 0} days), skipping technicals`,
    );
    return null;
  }

  const closes = candles.map((q) => q.close).filter((c): c is number => c != null);
  ...
  const volumes = candles.map((q) => q.volume).filter((v): v is number => v != null && v > 0);
  ...
  const atrResult = computeATR(candles);
  const stochResult = computeStochastic(candles);
  const obvTrend = computeOBVTrend(candles);
